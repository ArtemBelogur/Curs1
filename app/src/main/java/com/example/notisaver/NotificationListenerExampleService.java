...
public class NotificationListenerExampleService extends NotificationListenerService {
    ...
    public static final class InterceptedNotificationCode {
        ...
        public static final int OTHER_NOTIFICATIONS_CODE = 4; // We ignore all notifications with code == 4
    }
        ...
        if(activeNotifications != null && activeNotifications.length > 0) {
            for (StatusBarNotification activeNotification : activeNotifications) {
                if (notificationCode == matchNotificationCode(activeNotification)) {
                    Intent intent = new Intent("com.example.notisaver");
                    intent.putExtra("Notification Code", notificationCode);
                    intent.putExtra("package name", sbn.getPackageName());
                    intent.putExtra("user", sbn.getUser());
